...
from etc.engine import EtcEngine
from etc.file_utils import (
    is_file_stable,
    read_text_file,
    read_json_file,
    write_text_file,
...
class EtcFollowupWatcher:
    ...
    def _print_block(self, filename, followup_answer, final_answer):
        print()
        print("=" * 60)
        print("[ETC-FOLLOWUP] 새 후속 답변 감지: {}".format(filename))
        print()
    ...
    def process_file(self, file_path: Path):
        ...
        try:
            ...
            print()
            print("=" * 60)
            print("[ERROR] etc followup 처리 실패: {}".format(file_path.name))
            print(e)
            print("=" * 60)
